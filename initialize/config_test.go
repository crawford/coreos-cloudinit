...
func (tum *TestUnitManager) PlaceUnit(unit *config.Unit, dst string) error {
...
func (tum *TestUnitManager) MaskUnit(unit *config.Unit) error {
	tum.masked = append(tum.masked, unit.Name)
	return nil
}
func (tum *TestUnitManager) UnmaskUnit(unit *config.Unit) error {
