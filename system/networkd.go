...
func RestartNetwork(interfaces []network.InterfaceGenerator) (err error) {
	...
	if err = probe8012q(interfaces); err != nil {
		return
	}
	if err = probeBonding(interfaces); err != nil {
		return
	}
...
func probe8012q(interfaces []network.InterfaceGenerator) error {
	for _, iface := range interfaces {
		if iface.Type() == "vlan" {
...
func probeBonding(interfaces []network.InterfaceGenerator) error {
	args := []string{"bonding"}
	for _, iface := range interfaces {
		if iface.Type() == "bond" {
