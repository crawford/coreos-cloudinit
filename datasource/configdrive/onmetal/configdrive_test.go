	...
	tests := []struct {
		...
		},
		{
			...
			files: test.MockFilesystem{
				...
				"/media/configdrive/openstack/latest/vendor_data.json": "",
			},
			metadata: datasource.Metadata{
				Hostname: "host",
		...
		},
		{
			...
			files: test.MockFilesystem{
				"/media/configdrive/openstack/latest/meta_data.json":   `{"hostname": "host", "network_config": {"content_path": "config_file.json"}, "public_keys":{"1": "key1", "2": "key2"}}`,
				"/media/configdrive/openstack/latest/vendor_data.json": `{"network_info": {"something": "make it work"}}`,
			},
			metadata: datasource.Metadata{
				Hostname:      "host",
				NetworkConfig: []byte(`{"something": "make it work"}`),
				SSHPublicKeys: map[string]string{
					"1": "key1",
					"2": "key2",
				},
